...
// External pool set by server.js via setPool(); falls back to creating its own
// for standalone use (e.g. daily-check.js via GitHub Actions).
let pool = null;
...
async function ensureSchema() {
  ...
  // Schema migrations — safe to re-run (IF NOT EXISTS / idempotent)
  await p.query(`
...
export async function insertSnapshot(rateData) {
  await ensureSchema();
  const client = await getOrCreatePool().connect();
  try {
...
export async function getTodayOffers(date, category, territoryId, rateCode) {
  await ensureSchema();
  const { rows } = await getOrCreatePool().query(`
...
export default { setPool, getPool, insertSnapshot, getHistory, getSCOHistory, getSupplierHistory, getSummary,
